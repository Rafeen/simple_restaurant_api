...
pub async fn insert_order_item(
    ...
    new_order: Json<Vec<NewOrderItem>>,
    table_id: i32,
) -> Result<Status, Error> {
...
pub async fn fetch_order_item_by_item(
    db: &rocket::State<sqlx::PgPool>,
    table_id: i32,
    item_id: i32,
    ...
    let table = sqlx::query_as::<_, Table>(&query)
        .fetch_optional(&**db)
        .await
        .unwrap();
    match item {
...
pub async fn fetch_all_order_item_for_table(
    db: &rocket::State<sqlx::PgPool>,
    table_id: i32,
) -> sqlx::Result<Vec<OrderItem>> {
    let query = format!("SELECT * from tables where id={} ;", table_id);
...
pub async fn fetch_remaining_order_item_for_table(
    db: &rocket::State<sqlx::PgPool>,
    table_id: i32,
) -> sqlx::Result<Vec<OrderItem>> {
    let query = format!("SELECT * from tables where id={} ;", table_id);
...
pub async fn delete_order_item_for_table(
    db: &rocket::State<sqlx::PgPool>,
    table_id: i32,
    item_id: i32,
) -> sqlx::Result<Option<OrderItem>> {
...
pub async fn delete_all_orders_for_table(
    db: &rocket::State<sqlx::PgPool>,
    table_id: i32,
...
pub async fn is_duplicate_order(
    db: &rocket::State<sqlx::PgPool>,
    table_id: i32,
    ...
    for item in order_items.to_vec() {
        let query = format!(
            "SELECT * from order_items where table_id={} and item_id={} ;",
